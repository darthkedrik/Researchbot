...
TEST_FILE       = DIR_PATH + "corpus.csv"          # set this to the new test file name
PREDICTION_FILE = DIR_PATH + "only_b_insult_preds.csv"                 # predictions will be written here 
...
def ngrams(data, labels, ntrain, mn=1, mx=1, nm=500, binary = False, donorm = False, stopwords = False, verbose = True, analyzer_char = False):
    ...
    if binary:
        vectorizer = CountVectorizer(ngram_range=(mn,mx),binary=True)
    elif stopwords:
...
def run(verbose = True):
    ...
    test_data = readCsv2(TEST_FILE,chan_only_b)
    test_data = [x[5] for x in test_data]        
...
def readCsv2(fname, sublist, delimiter = ","):
    reader = csv.reader(open(fname,"r"),delimiter=delimiter)
